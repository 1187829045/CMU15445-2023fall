...
class ExtendibleHTableHeaderPage {
 public:
  ...
  void Init(uint32_t max_depth =
                HTABLE_HEADER_MAX_DEPTH);  //冲缓冲池中创建一个头页必须调用该函数去设置最大深度，若没有参数就是默认值
  ...
  auto HashToDirectoryIndex(uint32_t hash) const
      -> uint32_t;  //函数接受键的哈希值作为输入，并返回表示该键映射到的目录下标。
   ...
   */
  void SetDirectoryPageId(uint32_t directory_idx,
                          page_id_t directory_page_id);  //设置directory_page_id在 directory_idx下标下
