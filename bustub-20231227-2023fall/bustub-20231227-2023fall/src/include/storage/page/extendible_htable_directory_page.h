...
class ExtendibleHTableDirectoryPage {
 public:
   ...
   */
  void Init(uint32_t max_depth = HTABLE_DIRECTORY_MAX_DEPTH);//冲缓冲池中创建一个目录页必须调用该函数去设置最大深度，若没有参数就是默认值
   ...
   */
  auto HashToBucketIndex(uint32_t hash) const -> uint32_t;///函数接受键的哈希值作为输入，并返回表示该键被映射到的桶索引下标。
